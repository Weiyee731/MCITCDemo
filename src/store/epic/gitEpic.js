...
const url = "https://cms.mcitc.my/eCommerceCMS_DEV/api/mcitc/";
const platformType = "MCITC";
const PROJECTID = 1;
...
export class GitEpic {
  ...
  getBanner = action$ =>
    action$.pipe(filter(action => action.type === GitAction.GetBanner), map(action => {
      return dispatch => {
        try {
          return fetch(url + "General_ViewBanner?PROJECTID=" + PROJECTID)
            .then(response => response.json())
            .then(json => {
              json = JSON.parse(json)
              if (json[0].ReturnVal === 1) {
  ...
  getAllCategories = action$ =>
    action$.pipe(filter(action => action.type === GitAction.GetProductCategory), map(action => {
      return dispatch => {
        try {
          return fetch(url + "Product_CategoryListByAll?PROJECTID=" + PROJECTID)
            .then(response => response.json())
            .then(json => {
              json = JSON.parse(json)
              if (json[0].ReturnVal === 1) {
  ...
  getAllCategoriesListing = action$ =>
    action$.pipe(filter(action => action.type === GitAction.GetProductCategoryListing), map(action => {
      return dispatch => {
        try {
          return fetch(url + "Product_CategoryListing?ProjectID=" + PROJECTID)
            .then(response => response.json())
            .then(json => {
              json = JSON.parse(json)
              if (json[0].ReturnVal === 1) {
      ...
      return dispatch => {
        try {
          return fetch(url +
            ...
            "&PROJECTID=" + PROJECTID + "&PaymentType=" + action.payload.paymentType)
            .then(response => response.json())
            .then(json => {
              json = JSON.parse(json)
              if (json[0].ReturnVal === 1) {

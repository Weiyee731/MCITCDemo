...
import PromotionDetailsComponent from "../../pages/promotionDetails/promotionDetails.component";
...
const DeletableTableToolbar = (props) => {
  ...
  const onDeletePromotion = () => {
    props.PromotionProps.CallDeletePromotion(props.selectedData);
    setTimeout(
      function () {
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          Please select the promotion that you want to delete.
        </Typography>
      )}
...
function DeletableTable(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props.Data.map((n) => n.PromotionID);
    ...
    <div>
      <Paper className={classes.paper}>
        <DeletableTableToolbar
          ...
          PromotionProps={props.PromotionProps}
        />
        <TableContainer>
          <Table
              ...
              {stableSort(props.Data, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.PromotionID)}
                      role="checkbox"
                    ...
                    >
                      ...
                      {/* <TableCell align="center">
                            {row.PicturePromotion}
                                
                              </TableCell> */}
                      <TableCell align="left">{row.PromotionTitle}</TableCell>
                      <TableCell align="left">{row.BeginDate}</TableCell>
                      <TableCell align="left">{row.EndDate}</TableCell>
                      <TableCell align="left">{row.PromotionDesc}</TableCell>
                      <TableCell align="left">
                        {row.PromotionDetail
        ...
        {this.state.detailsShown ? (
          ...
          <div>
            ...
            {this.props.Data.filter((searchedItem) =>
              searchedItem.PromotionTitle.toLowerCase().includes(
                this.state.searchFilter
              )
            ).map((filteredItem) => {
              filteredPromotion.push(filteredItem);
            })}
            <DeletableTable
              Data={filteredPromotion}
              PromotionProps={this.props.PromotionProps}
            ></DeletableTable>
          </div>
        ) : (
          <div>
...
class ViewProductPromotionComponent extends Component {
  constructor(props) {
    ...
    // change this one to callALLPromotionsByPromotionStatus later on

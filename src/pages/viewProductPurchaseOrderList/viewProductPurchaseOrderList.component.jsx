...
function mapStateToProps(state) {
  return {
    ...
    allpurchaseorders: state.counterReducer["purchaseOrders"],
  };
}
...
class DisplayTable extends Component {
  ...
  received = (row) => {
    this.props.ProductProps.CallUpdatePurchaseOrderStatus(row);
        ...
        {this.state.detailsShown ? (
          ...
          <div>
            ...
            <div>
              <Button>
                <i class="fa fa-plus" aria-hidden="true"></i>
                <Link className="nav-link" to={"/viewProductPurchaseOrder"}>
                  Create new Purchase Order
                </Link>
              </Button>
              <FormControlLabel
...
class viewProductPurchaseOrderList extends Component {
  constructor(props) {
    ...
    //       <div style={{ width: "100%" }}>
    //         <DisplayTable
    //           Data={this.props.allpurchaseorders}
  ...
  render() {
    ...
    if (allPurchaseOrdersDate.length > 0) {
      console.log(allPurchaseOrdersDate);
      var generateTabs = allPurchaseOrdersDate.map((status, i) => {
        return (
          <Tab
      ...
      var generatePanels = this.state.ProductPurchaseOrderStatus.map(
        (status, i) => {
          var allPurchaseOrdersList = this.props.allpurchaseorders;
          allPurchaseOrdersList = allPurchaseOrdersList.filter(
            (items) =>
              items.ProductPurchaseOrderStatus ==
